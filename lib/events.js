...
var Commands = [];
var mental;
if (config.HANDLERS == 'false') mental = '^'
else mental = config.HANDLERS
var x3;
if (!mental.startsWith('^[') && !mental === '^') x3 = '^[' + mental + ']'
else x3 = mental
function addCommand(info, func) {
    ...
    if (info['on'] === undefined && info['pattern'] === undefined) {
        ...
        if (info['pattern'] !== undefined) {
            infos.pattern = new RegExp((info['handler'] === undefined || info['handler'] === true ? x3 : '') + info.pattern, (info['flags'] !== undefined ? info['flags'] : ''));
        }
    } else {
        infos.pattern = new RegExp((info['handler'] === undefined || info['handler'] === true ? x3 : '') + info.pattern, (info['flags'] !== undefined ? info['flags'] : ''));
    }
